...
php 002_make_train_upos.php

echo "Train on UPOS+LEMMA"
chmod u+x 003_train.sh
time ./003_train.sh
...
chmod +x 005_run_model_upos.sh
time ./005_run_model_upos.sh

echo "Prepare train corpus XPOS"
php 006_make_train_xpos.php

echo "Train on XPOS+LEMMA"
chmod u+x 007_train_xpos.sh
time ./007_train_xpos.sh

echo "Run model XPOS"
chmod u+x 008_run_model_xpos.sh
